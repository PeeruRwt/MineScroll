...
darkModeToggle.addEventListener("change", function () {
    ...
    if (this.checked) {
        ...
        cursor.style.backgroundColor = localStorage.getItem('cursorColor') || 'black';
    }
});
...
function generateRandomContent() {
    ...
    const words = ['I am 21', 'ipsum', 'dolor', 'Dollar bill', 'amet', 'consectetur', 'Broke', 'elit', 'sed', 'do', 'eiusmod', 'Peeru', 'incididunt', 'ut', 'labore', 'et', 'dolore', 'magna', 'aliqua'];
...
function getRandomSpeed() {
    ...
    const fractionalValues = [
        ...
        20.5, 20.8, 21.1, 21.4, 21.7, 22.0, 22.3, 22.6, 22.9, 23.2, 23.5, 23.8, 24.1, 24.4, 24.7, 25.0, 25.3, 25.6, 25.9, 
        26.2, 26.5, 26.8, 27.1, 27.4, 27.7, 28.0, 28.3, 28.6, 28.9, 29.2, 29.5, 29.8,
    ];
    ...
    if (!isHardMode) {
        if (score <= 50) {
            ...
            baseSpeed = 15 + (post50Progress * 12); 
        }
    } else { 
        if (score <= 50) {
            const progress = score / 50;
            baseSpeed = 1 + (progress * 29);
        } else {
            baseSpeed = 30;
        }
    }
...
function changeCursorColor() {
    ...
    setTimeout(() => {
        cursor.style.backgroundColor = localStorage.getItem('cursorColor') || (document.body.classList.contains('dark-mode') ? 'white' : 'black');
...
document.getElementById("startPlayButton").addEventListener("click", () => {
